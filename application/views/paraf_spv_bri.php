	...
	<div class="row">
		<div class="col-md-12">
			...
			<header class="panel-heading">
				...
				<header class="panel-heading">
				<p>Aktifitas Tanggal <?php echo date("d-m-Y H:m:s");?></p>

				<a href="<?php echo site_url('task/paraf_spv_bri_uncheckonly'); ?>" class="btn btn-success btn-sm"><i class="fa fa-out"></i>uncheck only</a>
				</header>
				<div class="table-responsive">
					...
					<thead>
						...
						<tbody>
					<?php
					//$activity_listpending 
					if ($activity_listpending){
						echo "<p><font color='red'>Mohon selesaikan task pending terlebih dahulu</font></p>";
					} else {
							...
							<td>
								...
								if ($jamx != ""){
									...
									if ($real < 0) {
										...
										?>
										<p id="<?php echo $list['id_act']?>a" style="display:none"><?php echo date('Y-m-d H:i:s', $newtimestamp);?></p>
										<p id="<?php echo $list['id_act']?>b" style="display:none"><?php echo $list['id_act']?></p>
										<a href="#" data-toggle="class" class="btn btn-white btn-xs approve" title="Klik untuk approve!"><i class="fa fa-check-square-o text-muted text"></i><i class="fa fa-check-square-o text-success text-active"></i></a>
...
$('.approve').click(function(e){
	if($(this).hasClass('active') == false){
		...
		//alert(a);
		$.getJSON('<?php echo site_url('task/input_approve_permohonan'); ?>', {id_act:b,time:a});		
	}
});
</script>

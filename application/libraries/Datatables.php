<?php if(!defined('BASEPATH')) exit('No direct script access allowed');
  ...
  */
  class Datatables
  {
	...
	*/	
	if($this->ci->input->get_post('iSortCol_0'))
	{
		for($i=0; $i<intval($this->ci->input->get_post('iSortingCols')); $i++)
		{
			$iSortCol = $this->ci->input->get_post('iSortCol_'.$i);
			$bSortable = $this->ci->input->get_post('bSortable_'.intval($iSortCol));
			$sSortDir = $this->ci->input->get_post('sSortDir_'.$i);
			$mDataProp = $this->ci->input->get_post('mDataProp_'.$iSortCol);

			if($bSortable == 'true')
			{
				$this->ci->db->order_by($mDataProp, $sSortDir);
			}
		}
	}
    ...
    */
    private function get_filtering()
    {
      ...
      foreach($this->filter as $val)
		...
		$temp_first = TRUE;
		if($this->ci->input->get_post('sSearch'))
        {
            for($i=0; $i<intval($this->ci->input->get_post('iColumns')); $i++)
            {
                $bSearchable = $this->ci->input->get_post('bSearchable_'.$i);
                ...
                if(isset($bSearchable) && $bSearchable == 'true')
                {
					if($temp_first) 
					{
						$this->ci->db->start_group_like($this->ci->input->get_post('mDataProp_'.$i), $this->ci->input->get_post('sSearch'), 'both', '');
						$temp_first = FALSE;
					}
					else
					{
						$this->ci->db->or_like($this->ci->input->get_post('mDataProp_'.$i), $this->ci->input->get_post('sSearch'));
					}
                }
            }
